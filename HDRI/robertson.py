...
class RobertsonHDR:
    ...
    def optimize_E(self, g_func, channel): 
        ...
        # numpy method
        # w = self.weight[self.Z[..., channel]]
        # t = self.expo_time
        # upper_sum = np.array(w * g_func[self.Z[..., channel]] * t)
        # lower_sum = np.array(w * t * t)

        # return np.sum(upper_sum / lower_sum)
    ...
    def optimize_g(self, E_func, channel):
        ...
        Em_1 = np.sum(self.Z[t_idx,: ,: , channel] * self.expo_time[t_idx] for t_idx in range(len(self.expo_time)))
